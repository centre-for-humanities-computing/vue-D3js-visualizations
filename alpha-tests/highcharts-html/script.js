...
/**
 * Wraps a general function for the initial highchart tests.
 */
function HighChartsWrapper(graphID, graphData, graphTitle = '', graphSubtitle = '', graphY = '') {
	let arrayData = CSVToArray(graphData)
	console.log(arrayData)
	console.log(SelectColumns(arrayData, [1]))
	Highcharts.chart(graphID, {
		...
		series: [{
			...
			// name: 'Cases regression',
			// color: 'rgba(223, 83, 83, .5)',
			data: SelectColumns(arrayData, [1])
		}, {
			yAxis: 1,
			type: 'scatter',
			// regression: true,
			...
			// name: 'Cases regression',
			// color: 'rgba(223, 83, 83, .5)',
			data: SelectColumns(arrayData, [2])
		}],
 ...
 * @returns columns = [[]]
 */
function SelectColumns(data, indices) {
	// Assuming data style [[,],[,],...]
	columns = [
		[]
	]
	...
	for (row in data) {
		...
		selected = []
		for (i in indices)
			selected.push(data[row][indices[i]])
		columns.push(selected)
	}

	return columns.splice(0, )
}

/**
 * This parses a delimited string into [[]].
 * @params strData = `csv,csv\ncsv,csv`
 * @returns data = [[]]
 */
// ref: http://stackoverflow.com/a/1293163/2343
function CSVToArray(strData, index, strDelimiter) {
